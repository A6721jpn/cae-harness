...
import pytest
from intent_fixture import _created, _evidence, _quality_registration, complete_spec
...
def test_public_intent_answer_freeze_and_explicit_E_edit(
    ...
    def http(endpoint: str, body: dict[str, Any], *_: Any) -> dict[str, Any]:
        ...
        return (
            ...
            else response(proposal_for(body))
        )
    ...
    draft = service.set_spec(
        created.case_id,
        values=PartialCaseSpec(budget=Budget(Quantity(15, "s"), 1, 1, 2, 2200)),
    ...
    parent = service._storage(created.case_id).get_revision(created.case_id, frozen["revision_id"])
    code, edited = invoke(
        capsys,
        created,
        path,
        ...
        draft.generation,
        "edit",
        "material.youngs_modulus = 2 MPa",
        "--base",
        parent.revision_id,
    )
    assert code == 0 and edited["draft"]["parent_revision_id"] == parent.revision_id
    ...
    assert (
        service._storage(created.case_id).current_frozen_revision(created.case_id)
        == parent.revision_id
    )


def test_negation_conflict_and_wrong_entity_never_ground(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    service, created, path = setup_case(tmp_path, monkeypatch)
...
def test_stale_before_spend_and_during_response_retains_accounting(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    service, created, path = setup_case(tmp_path, monkeypatch)
    calls: list[str] = []
...
def test_numerical_diagnostics_and_uncertain_reentry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    service, created, path = setup_case(tmp_path, monkeypatch)
    bad = settings()
    bad["output_tokens"] = 15
    path.write_text(json.dumps(bad), encoding="utf-8")
    assert (
        invoke(
            capsys, created, path, "intent", 1, "bad", "material.model = isotropic_linear_elastic"
        )[0]
        == 2
    )
    path.write_text(json.dumps(settings()), encoding="utf-8")
    ...
    assert (
        invoke(
            ...
            1,
            "timeout",
            "material.model = isotropic_linear_elastic",
        )
        == result
    )
    assert len(calls) == 1
    ...
    validation = service.validate_case(created.case_id)
    assert validation.status == "INVALID_INPUT"
    assert not any(item.code.value == "needs_physical_input" for item in validation.diagnostics)
