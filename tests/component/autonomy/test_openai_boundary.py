...
def settings() -> dict[str, Any]:
    return {"provider": "openai_responses", "model": "explicit-test-model",
            "key_env": "P4_PRIVATE_TEST_KEY", "budget": Budget(Quantity(15, "s"), 1, 1, 2, 2200).to_dict(),
            "input_tokens": 1000, "output_tokens": 200, "socket_seconds": 3}


def response(assignments: list[dict[str, str]] | None = None, status: str = "completed") -> dict[str, Any]:
    import json
    return {"status": status, "model": "returned-test-model", "usage": {"input_tokens": 100, "output_tokens": 20, "total_tokens": 120},
            "output": [{"type": "reasoning", "summary": []}, {"type": "message", "role": "assistant", "status": "completed",
            "content": [{"type": "output_text", "text": json.dumps({"assignments": assignments or []})}]}]}


def common() -> dict[str, Any]:
    return {"model": "explicit-test-model", "instructions": "bounded proposal", "input": "explicit source",
            "text": {"format": {"type": "json_schema", "name": "intent_proposal", "strict": True,
...
def test_count_generation_identity_and_usage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    output = adapter.exchange(common(), settings(), before_generation=lambda: None, on_event=events.append)
    ...
    assert {k: calls[1][1][k] for k in ("store", "background", "stream")} == dict.fromkeys(("store", "background", "stream"), False)
    assert calls[1][1]["tools"] == [] and calls[1][1]["max_output_tokens"] == 200
    assert any(e.get("usage", {}).get("total_tokens") == 120 for e in events)


def test_overcap_refusal_incomplete_and_timeout_do_not_propose(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(adapter, "_resolve_key", lambda _: "private-injected")
    for outcome in ("overcap", "refusal", "incomplete", "timeout"):
        calls: list[str] = []
        events: list[dict[str, Any]] = []
        def http(path: str, *_: Any) -> dict[str, Any]:
            calls.append(path)
            if outcome == "timeout":
        ...
        with pytest.raises((ValueError, TimeoutError)):
            adapter.exchange(common(), settings(), before_generation=lambda: None, on_event=events.append)
        assert len(calls) == (1 if outcome in {"overcap", "timeout"} else 2)
        if outcome in {"refusal", "incomplete"}:
            assert any(e.get("usage", {}).get("total_tokens") == 120 for e in events)


def test_python_only_owned_transport_deadline(tmp_path: Path) -> None:
    from febio_cae.adapters.geometry.preparation import _run_owned
    started = time.monotonic()
    with pytest.raises(TimeoutError):
        _run_owned((sys.executable, "-c", "import time; time.sleep(30)"), tmp_path,
                   timeout_seconds=0.4, memory_bytes=256 * 1024 * 1024)
