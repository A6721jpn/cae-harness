...
def test_meshing_first_consumer() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-I", "-c", f"import sys; sys.path.insert(0, {str(source)!r}); "
...
def test_box_volume_and_complete_planar_boundary(mesh_port: Any, synthetic_case_revision: Any) -> None:
    ...
    for face in artifact.faces:
        ...
        planes = [(axis, limit) for axis in range(3) for limit in limits[axis]
...
def test_folded_quadratic_mapping_rejected(
    mesh_port: Any, synthetic_backend: Any, synthetic_case_revision: Any, monkeypatch: Any,
) -> None:
    ...
    def folded(*args: Any) -> Any:
        ...
        return replace(mesh, nodes=tuple(
