...
def _case() -> tuple[CaseRevision, MeshArtifact, CompatibilityProfile]:
    ...
    controls = (
        ...
        SolverControl("dtol", Quantity(.002, "1")),
        ...
        SolverControl("tolerance", Quantity(.15, "1")),
        SolverControl("gaptol", Quantity(.02, "mm")),
        SolverControl("search_tol", Quantity(.03, "1")),
        SolverControl("search_radius", Quantity(2, "mm")),
        SolverControl("two_pass", False),
    )
    spec = replace(revision.spec, solver_policy=replace(
        revision.spec.solver_policy, controls=controls,
    ...
    vertices = (
        ((0., 0., 0.), (0., .01, 0.), (.01, 0., 0.), (0., 0., -.01)),
        ((0., 0., .001), (.01, 0., .001), (0., .01, .001), (0., 0., .011)),
    )
    nodes = []
    for offset, corners in zip((0, 10), vertices, strict=True):
        mids = tuple(tuple((corners[a][i] + corners[b][i]) / 2 for i in range(3))
    ...
    profile = replace(profile, output_mappings=tuple(
...
def _compile(tmp_path: Path, revision: CaseRevision, mesh: MeshArtifact, profile: CompatibilityProfile) -> tuple[ExecutionBundle, ET.Element, bytes]:
...
def test_complete_two_body_compiler_port_emits_native_entities_and_references(tmp_path: Path) -> None:
    ...
    assert float(root.findtext("Control/time_steps", "nan")) * float(root.findtext("Control/step_size", "nan")) == pytest.approx(1)
    ...
    emitted_nodes = {int(node.attrib["id"]): tuple(float(x) for x in (node.text or "").split(","))
    ...
    for element in root.findall("Mesh/Elements/elem"):
        expected = next(item for item in mesh.elements if item.element_id == int(element.attrib["id"]))
    ...
    node_sets = {item.attrib["name"]: set(_integers(item.text)) for item in root.findall("Mesh/NodeSet")}
    ...
    assert [fixed.findtext(axis) for axis in ("Rx_dof", "Ry_dof", "Rz_dof", "Ru_dof", "Rv_dof", "Rw_dof")] == ["1", "1", "0", "1", "1", "1"]
    ...
    assert bundle.argv[1:] == ("-i", "input/case.feb", "-o", "output/solver.log", "-p", "output/results.xplt", "-noappend", "-noconfig")
    ...
    assert settings["compatibility_profile_digest"] == hashlib.sha256(profile.to_bytes()).hexdigest()
...
def test_signed_rigid_motion_is_evaluated_in_the_declared_direction(tmp_path: Path) -> None:
    revision, mesh, profile = _case()
    revision = replace(revision, spec=replace(revision.spec, motion=replace(
        revision.spec.motion, direction=replace(revision.spec.motion.direction, z=-1),
    )))
    ...
    values = [tuple(float(x) for x in (p.text or "").split(",")) for p in curve.findall("points/point")]
    assert values == [(0., 0.), (1., .0001)]
    assert float(value.text or "nan") * values[-1][1] == pytest.approx(-.0001)


def test_numeric_contact_and_renamed_resolved_sets_are_not_lost(tmp_path: Path) -> None:
    revision, mesh, profile = _case()
    friction = CoulombFriction(Quantity(.37, "1"), evidence("contact.friction_model", "coulomb"), evidence("contact.friction_coefficient", "mu"))
    revision = replace(revision, spec=replace(revision.spec, contact=replace(revision.spec.contact, friction=friction)))
    mesh = replace(mesh, sets=tuple(replace(item, set_id="renamed-" + item.set_id) for item in mesh.sets))
    _, root, _ = _compile(tmp_path, revision, mesh, profile)
    contact = _required(root, "Contact/contact")
    assert float(contact.findtext("fric_coeff", "nan")) == pytest.approx(.37)
    assert float(contact.findtext("penalty", "nan")) == 7.5
    assert float(contact.findtext("search_radius", "nan")) == pytest.approx(.002)
    assert float(contact.findtext("gaptol", "nan")) == pytest.approx(.00002)
...
@pytest.mark.parametrize("invalid", ["solver-version", "capability-version", "compression", "native-location", "unknown-control", "missing-penalty", "penalty-unit"])
def test_unsupported_profile_and_numerics_fail_before_staging(tmp_path: Path, invalid: str) -> None:
    ...
    if invalid == "solver-version":
        ...
        capability = replace(capability, **({"version": "unknown"} if invalid == "capability-version" else {"compression": "zlib"}))
        profile = replace(profile, capabilities=(capability, *profile.capabilities[1:]))
    elif invalid == "native-location":
        profile = replace(profile, output_mappings=tuple(replace(item, native_name="reaction forces") if item.location == "rigid_body" else item for item in profile.output_mappings))
    else:
        ...
        if invalid == "unknown-control":
            ...
            controls = tuple(replace(item, value=Quantity(7.5, "m")) if item.name == "penalty" else item for item in controls)
        revision = replace(revision, spec=replace(revision.spec, solver_policy=replace(revision.spec.solver_policy, controls=controls)))
