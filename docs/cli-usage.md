...
| 残件 | native-enabled final flowの数値gate・baseline `COMPLETE`・Studio `LAUNCHED`・candidate comparisonは取得済み。合格済みsource＋native candidateは受理対象として記録し、MVPで残る必須記録だったmanual 8は2026-09-22のmanual final flowで完遂した（新規case、公開CLI手動22コマンド全exit 0、inspection1／preparation1／solver2／Studio1、[manual 8実行記録](reviews/2026-09-22-mvp-manual-final.md)）。V2 integrationは明示的ユーザー指示待ちとする。旧manual `case-6294a0a9e02c`はretry／reset／代替caseを行わず保持する。完了済み新wheel smokeと各fresh flow recordを分けて保持する。[canonical review](reviews/2026-09-20-mvp-planar-e2e.md) |
