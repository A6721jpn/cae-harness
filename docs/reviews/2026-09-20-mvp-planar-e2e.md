...
manual `case-6294a0a9e02c`の今回の`FAILED`／`ABORTED/BLOCKED`は別境界としてretry／reset／代替caseを一切行わず、旧251/b28のE2E、missing-Gmsh／runner setup失敗、native-enabled final flow、新wheel smokeは別recordとして保持する。native-enabled automated 8-stepはPASS済みであり、MVPで残る必須記録はmanual 8である。candidate preview／Studio `CONFIRMED`はこのMVP manual gateとは別の表示境界として記録し、MVP全体／project doneは主張しない。
...
| MVP | 自動8手順は`PASS`。MVP全体はmanual 8 preparation未完了のみで`UNVERIFIED/BLOCKED`。native candidateは受理対象として記録し、manual caseはretry／reset／代替caseを行わず、この状態を保持する |
