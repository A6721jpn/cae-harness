...
文書版: 0.3 / 作成日: 2026-09-07 / 更新日: 2026-09-14 / 状態: V2開発中の工程・受け入れ基準。
...
PM兼PdM（`gpt-6-astra` / `high`）が優先順位・範囲・技術方針・受け入れ基準・有限予算を決定し、Luna Spawnで作成・続行する独立した実装ワーカー（`gpt-5.6-luna` / `max`）が担当ファイルを実装し、別タスクのレビュー担当（`gpt-6-astra` / `medium`）が検査する。P0〜P1の共通契約は凍結まで単一所有者が管理し、境界の固定後にinput/model、solver/FBS、autonomy、build/launchを別々のworktreeへ分離する。依存工程の証拠が必要な後続経路は、その工程が合格するまでは完成扱いしない。
...
### 5.4 同一caseの登録済みメッシュ収束経路

平面・線形弾性・摩擦なしboxの初期数値品質経路では、事前宣言した3サイズを同じcaseで
coarse→refined→fineの順に生成・実行する。`prepare-planar --parent-revision-id`は
現在の登録済みPREPARED親からglobal_sizeだけを変えた子版を生成し、各meshと結果を
独立した生成記録へ結び付ける。親や古い結果の置換、別caseの寄せ集め、
古い生成receiptによる新しいmeshの承認は禁止する。

この経路の永続case-wide上限をmesh3回・FEBio4回とし、失敗予約も消費する。
第四のFEBio実行はfine版のYoung率のみの変更に用いる。CLIの収束前2版は品質未確定を
維持し、fine版の登録済み3サイズ比較と変更後の正規化比較が成立してから数値合格を
...
必要な数値根拠として、部品全体・治具全体の変位、治具全体の力と絶対位置、
明示固定支持領域の反力を要求する。メッシュ比較の治具力評価は`identity`集計で
全保存状態の曲線を保持し、peakだけへ縮約しない。幾何比較用のbody・要素座標は
JSON互換配列として正規化し、入力の列挙順に依存せず実際の座標変更を識別する。
...
| 役割 | モデル・設定 | 責務 |
|---|---|---|
| PM兼PdM | `gpt-6-astra` / `high`、独立会話タスク | 製品の優先順位・範囲・技術方針・受け入れ基準・有限予算を決定し、依存・証拠・レビュー・V2統合を管理する |
| 実装ワーカー | `gpt-5.6-luna` / `max`、Luna Spawnで作成・続行する独立タスク | 固定した境界と許可ファイルをテスト先行で実装し、cleanなコミットと検証記録を渡す |
| コードレビュー | `gpt-6-astra` / `medium`、独立会話タスク | baseとcandidateの正確なSHAを対象に読み取り専用で検査し、PMへ判定を返す |

共通契約は凍結まで単一所有者が管理し、固定した境界から担当ファイルとworktreeを分離する。既存の他担当の成果・未完了変更を保存し、同じファイルへの同時編集を避ける。PMは正確なbase SHA、許可ファイル、禁止変更、RED/GREENのコマンド、依存、完了条件、未検証事項、cleanなコミットでの引き渡し要件を作業票に記載する。PMは管理文書と統合を担当し、ワーカーの担当製品ファイルを同時編集しない。実装タスクの作成・続行にはLuna Spawnを使う。モデル・effortは実行メタデータで確認し、不明なら未検証として報告する。
...
メッセージのモデル・effort指定は宛先に適用する。作業指示はPM兼PdMへ`gpt-6-astra` / `high`、実装ワーカーへ`gpt-5.6-luna` / `max`、レビュー担当へ`gpt-6-astra` / `medium`を明示する。結果報告は宛先の設定を維持する。完了・失敗・判断依頼に応じて連絡し、重複ACKや変化のない待機ポーリングを行わない。

実装ワーカーは結果をPMへ返し、PMが正確な候補SHAと検証記録をレビュー担当へ渡す。レビュー担当は修正を実装せず、重大度・再現条件・根拠・未検証ゲート・ACCEPT/REJECTを返す。Blocking/Highの未解消指摘がある候補は統合しない。PMは指摘を担当のLunaタスクへ差し戻し、修正後の新しいSHAで再レビューする。管理文書の指摘はPMが修正して再レビューする。
...
公開STEP準備経路は、PM兼PdMが既存成果と依存を確認して有限の作業票を固定し、担当のLuna Spawnタスクへ渡す。新規native実行は対象入力・条件・予算・許可が揃ってから行う。体制改訂そのものを製品経路やP0〜P7の完了証拠にはしない。
