...
P0〜P1で実機適合性と共通契約を単一所有者が固定する。独立レビューで固定した契約境界から、input/model、solver/FBS、autonomy、build/launchの実装を別worktreeへ分離できる。着手可能な境界の固定とP1全体の終了を区別し、依存工程が合格するまでは後続の実行経路を完成扱いしない。
...
| STEPから実行入力 | installed CLIからSTEP調査、領域解決、独立した剛体・mesh生成、検証済み`.feb`と固定bundle生成までつながる | 必要な契約境界のレビュー済みSHAからinput/modelとsolverを並行実装する。未固定契約は分岐させない |
...
P0〜P1の未固定共通契約の実装担当ワーカーは一人に限定し、共通契約の所有を分裂させない。独立レビューで型・codec・エラー・所有境界を固定した領域は、P1全体の終了前でも、その契約を変更しない別ワーカーがadapter実装に着手できる。PMは各依頼に固定済みの正確なSHAと許可境界を記載し、未成立の依存を合格扱いしない。PMは管理・統合を担当し、同じ製品ファイルをワーカーと同時編集しない。領域別ワーカーの追加もLuna Spawnを使い、作成と続行の各要求でLuna/maxを明示する。モデル・effortは実行メタデータで確認し、不明なら未検証として報告する。
