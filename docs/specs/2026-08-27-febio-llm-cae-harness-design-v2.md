...
文書版: 0.2 / 作成日: 2026-09-07 / 更新日: 2026-09-14 / 状態: 実装用設計、一般的な実機適合性は未検証。
...
開発上の意思決定はPM兼PdM（`gpt-6-astra` / `high`）が担う。承認済み要求内で優先順位、範囲、技術方針、受け入れ基準、有限予算を決定し、製品動作・受け入れの変更は本書と実装計画書へ反映してから実装する。実装ワーカーはLuna Spawnスキルで作成・続行する独立した`gpt-5.6-luna` / `max`タスク、独立レビューは別タスクの`gpt-6-astra` / `medium`とする。共通契約は凍結まで単一所有者が管理し、境界の固定後に担当ファイルとworktreeを分離する。具体的な開発手順は[開発契約](../../AGENTS.md)と実装計画書10節に従う。これは開発体制であり、製品のLLM adapterのモデル要件を変更しない。
...
上記の既存mesh再利用契約に従う。
公開に失敗した派生版も、親指定なしの初回準備として置き換えてはならない。
失敗した初回ルート版の回復と区別し、受理済みの物理条件・予算をリセットしない。
...
`ResultManifest.files`は固定済み出力を列挙し、コンパイル済み入力を重複登録しない。
`input/case.feb`は同じ試行の登録済み`ExecutionBundle.files`から解決し、manifestの
`bundle_digest`との一致、入力role、保持先、サイズ、SHA-256を検証する。
...
登録readerが完全な出力辞書を要求するprofileでは、compilerは利用者の表示要求だけでなく
登録辞書の全変数を出力する。生データの存在だけを品質合格へ読み替えず、接触・運動・
支持反力の各検査は明示した出力要求と全保存状態の評価を必要とする。
...
misplaced or unknown interface records remain UNVERIFIED. Preserve the original
line/byte spans and all existing admission, final-cycle and failure precedence.
